...
#seperate dfr in two with mean and se------------------------------------------------------------------
  tab1_means <- table1[, c(2,4,6,8)]
  tab1_se <- table1[, c(3,5,7,9)]
  
  ###now paste together and round
  dat1 <- data.frame(paste0(sprintf("%2.1f",round(tab1_means[,1], 1)), " (", sprintf("%3.2f", round(tab1_se[,1],2)),")"))
  dat2 <- data.frame(paste0(sprintf("%2.1f",round(tab1_means[,2], 1)), " (", sprintf("%3.2f", round(tab1_se[,2],2)),")"))
  dat3 <- data.frame(paste0(sprintf("%2.1f",round(tab1_means[,3], 1)), " (", sprintf("%2.2f", round(tab1_se[,3],2)),")"))
...
cue <- read.csv("calculated data/CUEdaily.csv")  
  ...
  pve_table1[[3]] <- paste(pve_table1[[3]], siglet3[[2]][,1])
  pve_table1[[4]] <- paste(pve_table1[[4]], siglet3[[3]][,1])

  #add RLD seperate
  rldsigs <- read.csv("master_scripts/sigletters/sl_rld.csv") # order is this: 5,10,15,20,25,35,1000 
  
  pve_table1[[5]] <- paste(pve_table1[[5]][1:7], rldsigs[[1]][1:7])
  
  ###replace the free in RLD with nothing
  pve_table1[7,5] <- ""
  
  ##rld an volume p<0.0001
  pval_row <- c("Volume Effect (P value)", 0.001, 0.001, 0.009, 0.001, "")
