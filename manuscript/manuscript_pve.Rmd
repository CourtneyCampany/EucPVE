...
*also deleted below 2 sentences*
As plant growth cannot be simply determined by *A*~n~, growth models must take into account several simultaneous processes affecting the C balance at any given time [@fourcaud2008plant]. Whether growth limitation in woody species is driven mostly by *A*~n~ or other components of the C balance remains largely uncertain.

\
In previous studies, the sink limitation of growth has been explored by manipulations of both C source and C sink activity. Examples of experimental approaches that manipulate C source activity include elevated atmospheric CO2 concentration and partial defoliation experiments. Elevated CO~2~ causes an increase in leaf-level photosynthetic rate, *A*~n~ [@drake1997more; @ainsworth2007response]. While this increase is often observed to drive an increase in plant biomass production (eg any number of papers),  

across four forest Free-Air CO~2~ Enrichment (FACE) experiments this increased photosynthetic uptake resulted in a stimulation of biomass production by 23 % on average [@norby2005forest]. Evidence from a wide range of elevated CO~2~ experiments, however, also reveals that even with an average photosynthetic enhancement of over 30 %, the biomass growth rate only increases by around 10 % [@kirschbaum2011does]. 
...
We addressed questions regarding the coordination of *A*~n~ and growth by complementing empirical results with a C mass balance model. 

*Poorter review talks about water availability (rule out) temperature effects on soil volume (can rule out) nutrient availability (possible) root signaling (magic) but strangely, not carbohydrate feedback* 
...
*add exudation*
Alternatively, root exudation may have increased with reduced rooting volumes to facilitate N uptake in favour of increasing partitioning to root biomass.
...
```  
**Figure 6**. (a) Total C mass of harvested seedlings versus modelled net leaf carbon gain over 120 days. The dashed black line hows the 1:1 line and the dashed red line shows the seedling biomass that would be expected if all seedlings had the same carbon use efficiency as the freely grown seedling. (b) Carbon use efficiency (ration of final biomass to modelled net leaf C gain) of plants growth in different container sizes.
