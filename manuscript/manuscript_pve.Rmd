...
\
(1)
$$P_i = \frac{L~C_\mathrm{day'i}~\sigma - R}{\epsilon}$$

where *L* is total plant leaf area (m^2^), *C*~day,i~ is the predicted daily carbon assimilation (g C m^-2^ d^-1^, explained below), $\sigma$ is a self-shading parameter (explained below), $\epsilon$ is a biomass conversion efficiency parameter (g C g mass^-1^) and *R* is the mass based total respiration of all tissue components (g C d^-1^). Total respiration was calculated as 
