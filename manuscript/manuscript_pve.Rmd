...
2). As the finite pool of rooting volume and soil nutrients will decline faster in trees growing in small containers, we expected reductions in partitioning to fine root mass to be magnified with decreasing container size.
...
where L is total plant leaf area (m^2^), C~day,i~ is the predicted daily carbon assimilation (g C g mass^-1^ d^-1^), $\sigma$~s~ is a self shading parameter, $\epsilon$~c~ is a biomass conversion efficiency parameter and R is the mass based total respiration of all tissue components (g C d^-1^). Total respiration was calculated as 
