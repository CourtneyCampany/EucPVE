...
\
(1)
$$P_i(t) = \frac{L_i(t-1)~Cday_i}~\sigma - R_i}{\epsilon}$$

where *L*~i~ is modelled total plant leaf area (m^2^), *C*~day,i~ is the predicted daily carbon assimilation (g C m^-2^ d^-1^, explained below), $\sigma$ is a self-shading parameter (explained below), $\epsilon$ is a biomass conversion efficiency parameter (g C g mass^-1^) and *R*~i~ is the mass based total respiration of all tissue components (g C d^-1^). Here, i represents model runs for each experimental treatment (1:7). Total respiration was calculated as 

\
(2)
$$R_i(t) = \Sigma_{c=1}^{3}(R_\mathrm{c}~M_\mathrm{c,i})$$

where *R*~c~ is published or measured tissue respiration rates of each woody tissue component, including fine roots, coarse roots and stems (Table `r pvetabS`, g C g mass^-1^ d^-1^) and *M*~c,i~ is the modelled standing biomass of each component (g). Leaf respiration is included in the calculation of *C*~day~ (described below). The change in individual component biomass (*M*~c,i~), here solved on a daily time step, is given by

\
(3)
$$\frac{dM_\mathrm{c}}{dt} = \theta_\mathrm{c}~P_\mathrm{i}~-~(\Lambda_\mathrm{c}~M_\mathrm{c,i})$$
...
\
We then used this model to test the hypothesis that the effects of belowground sink limitation on rates of leaf *A*~n~ were sufficient to accurately predict overall seedling biomass production after 120 days. First, a default model with mean photosynthetic parameters from the free seedlings was optimized to estimate a LMF which correctly predicted both the leaf mass and total biomass of the harvested free seedling (*M*~free~). This optimized LMF was then used to constrain model runs with treatment-specific *C*~day~, while keeping woody tissue respiration and tissue turnover parameters constant, to determine if changes in leaf *A*~n~ alone could predict biomass of seedlings in containers (*M*~pots~). Cumulative net leaf C gain for each treatment was equal to the sum of each value of *C*~day,i~ over 120 days and final seedling C was assumed to equal half of the final dry mass for both modeled and observed seedlings. Seedling biomass production was compared between model output and harvested seedlings with treatment-specific mean *C*~day~ by first scaling values to the free seedling control.
