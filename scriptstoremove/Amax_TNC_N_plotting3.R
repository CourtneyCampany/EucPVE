...
#model and stats
Afit_full <- lmer(A_mass ~ nitro_mg+starch_perc+nitro_mg:starch_perc + (1|block/ID), data=photo_chem)
#####model parameters for plotting
f <- fixef(Afit_full)
anova(Afit_full)

###same model with fixed effects in different order for ease of plotting preductions later
Afit2_full <- lmer(A_mass ~ starch_perc+nitro_mg+starch_perc:nitro_mg + (1|block/ID), data=photo_chem)
f2 <- fixef(Afit2_full)
